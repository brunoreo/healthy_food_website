...
app.post("/api/alternative", async (req, res) => {
  ...
  try {
    ...
    // Try to extract the main alternative name (simplest approach: inside quotes)
    let alternativeName = answer.match(/"(.*?)"/)?.[1];
    if (!alternativeName) {
      // fallback: use first few words
      alternativeName = answer.split(".")[0];
    }
    ...
    // Return both the AI answer and the recipe link
    res.json({
      alternative: answer,
